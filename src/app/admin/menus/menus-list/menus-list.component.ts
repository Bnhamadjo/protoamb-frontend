...
@Component({
  ...
  template: `
    <div class="menus-list">
      ...
      <div class="grid-table" *ngIf="!loading && menus.length > 0">
        <div class="table-header">
          <span>Nome</span>
          <span>Localização</span>
          <span>Idioma</span>
          <span>Itens</span>
          <span class="center">Ações</span>
        </div>
        ...
        <div class="table-row card" *ngFor="let menu of menus">
          ...
          <div class="actions center">
            ...
            <a [routerLink]="['/admin/menus', menu.id]" class="btn sm">Configurações</a>
            <button (click)="deleteMenu(menu.id!)" class="btn danger sm">Eliminar</button>
          </div>
        </div>
      </div>

      <div *ngIf="!loading && menus.length === 0" class="empty">
...
export class MenusListComponent implements OnInit {
  ...
  loadMenus(): void {
    this.menuService.all().subscribe({
      ...
      error: () => { this.error = 'Falha ao carregar menus.'; this.loading = false; }
    });
  }

  deleteMenu(id: number): void {
    if (!confirm('Tem certeza que deseja eliminar este menu e todos os seus itens?')) return;
    this.menuService.delete(id).subscribe({
      next: () => this.menus = this.menus.filter(m => m.id !== id),
